...
use crate::models::{Transaction, TransactionType, ScanStatus};
use crate::db::{WalletAddressRepo, TransactionRepo, ScanStatusRepo};
    ...
    async fn scan_blocks(&self) -> Result<()> {
        ...
        let start_slot = {
            ...
            if let Some(status) = scan_status.as_ref() {
                ...
                current_slot.saturating_sub(100) // 默认扫描最近100个区块
            }
        };
        ...
        for slot in start_slot..=current_slot {
            if let Err(e) = self.scan_block(slot).await {
                error!("Error scanning block {}: {}", slot, e);
                continue;
            }
    ...
    async fn scan_block(&self, slot: u64) -> Result<()> {
        ...
        if let Some(transactions) = block.transactions {
            for tx in transactions {
                ...
                if let Err(e) = self.process_transaction(slot, &tx.transaction, tx.meta.as_ref()).await {
                ...
                match &ui_tx.message {
                    solana_transaction_status::UiMessage::Parsed(message) => {
                        ...
                        for instr in &message.instructions {
                            if let solana_transaction_status::UiInstruction::Parsed(parsed_ins) = instr {
                                match parsed_ins {
                                    solana_transaction_status::UiParsedInstruction::Parsed(pi) => {
                                        let program = pi.program.as_str();
                                        let parsed_val = &pi.parsed;
                                        if program == "system" {
                                            if parsed_val.get("type").and_then(|v| v.as_str()) == Some("transfer") {
                                                if let Some(info) = parsed_val.get("info") {
                                                    let from = info.get("source").and_then(|v| v.as_str()).unwrap_or("").to_string();
                                                    let to = info.get("destination").and_then(|v| v.as_str()).map(|s| s.to_string());
                                                    let lamports = info.get("lamports").and_then(|v| v.as_u64()).unwrap_or(0);
                                                    let amount = (lamports as f64) / 1_000_000_000f64;
                                                    if watched.contains(&from) || to.as_ref().map(|t| watched.contains(t)).unwrap_or(false) {
                                                        let tx_record = Transaction::new(
                                                            ...
                                                            fee_sol,
                                                            Utc::now(),
                                                            if meta.map(|m| m.err.is_none()).unwrap_or(false) { crate::models::TransactionStatus::Confirmed } else { crate::models::TransactionStatus::Failed },
                                                            Some(parsed_val.clone()),
                                                        );
                                                        let tx_repo = TransactionRepo::new(self.db.clone());
                                                        let _ = tx_repo.insert_transaction(&tx_record).await;
                                                        let _ = self.kafka_producer.send_transaction(&tx_record).await;
                                                    }
                                                }
                                            }
                                        } else if program == "spl-token" || program == "spl-token-2022" {
                                            let t = parsed_val.get("type").and_then(|v| v.as_str()).unwrap_or("");
                                            if t == "transfer" || t == "transferChecked" {
                                                if let Some(info) = parsed_val.get("info") {
                                                    ...
                                                    if let Some(v) = amount_raw {
                                                        if let Some(s) = v.as_str() { amount = s.parse::<f64>().unwrap_or(0.0); } else if let Some(n) = v.as_u64() { amount = n as f64; } else if let Some(n) = v.as_f64() { amount = n; }
                                                    }
                                                    if decimals > 0 { amount = amount / 10f64.powi(decimals as i32); }
                                                    let tx_type = if decimals == 0 && (amount - 1.0).abs() < f64::EPSILON { TransactionType::Nft } else { TransactionType::Token };
                                                    if watched.contains(&from) || to.as_ref().map(|t| watched.contains(t)).unwrap_or(false) {
                                                        let tx_record = Transaction::new(
                                                            signature.clone(),
                                                            slot,
